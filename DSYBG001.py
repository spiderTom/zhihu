...
def saveHtml( htmlUrl,imgName ="default.html" ):
    ...
    DstDir="D:\\zhihu\\1\\"
    targetFileName = DstDir+imgName
    print("save file: "+targetFileName+"\n")
    targetFileName = targetFileName.encode("gbk")
...
def downTopicViaMutiThread( filelist ):
    ...
    for (key,value) in filelist.items():
        filename = validateTitle(value)
        if isinstance(filename, str):
            filename = filename.decode('utf-8')

        filename += ".html"
        t = threading.Thread(target=saveHtml,args=(key,filename))
    ...
    for topic in soup.find_all('a'):
        if topic.has_attr("target") and topic.get("target") == "_blank":
            if topic.has_attr("href") and topic.get("href").find("question") != -1 and topic.get("href").find("answer") == -1:
                filelist[setting.prefixUrl + topic.get('href')] = topic.get_text()
